...
function numberFromForm(value: FormDataEntryValue | null): number | null {
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  const intent = cleanString(form.get("intent"));
  ...
  if (intent === "create") {
    const nom = cleanString(form.get("nom"));
    const instagram = cleanString(form.get("instagram"));
    const tiktok = cleanString(form.get("tiktok"));
    const type = cleanString(form.get("type"));
    const pays = cleanString(form.get("pays"));
    const produit = cleanString(form.get("produit"));
    const shippingStatus = cleanString(form.get("shippingStatus")) || "en_attente";
    const quantite = Number.parseInt(cleanString(form.get("quantite")), 10) || 1;
  ...
  if (intent === "import_csv") {
    const rowsRaw = cleanString(form.get("rows"));
    const rows: CsvRow[] = rowsRaw ? JSON.parse(rowsRaw) : [];
    ...
    for (const row of rows) {
      ...
      const csvPort = Number.parseFloat((row.fraisPortRaw ?? "").replace(",", "."));
      const port = Number.isFinite(csvPort) && csvPort > 0 ? csvPort : ugcShippingFromText(pays, row.produitRaw ?? "");
  ...
  return (
    <li style={{ display: "flex", alignItems: "center", gap: 7, padding: "3px 0" }}>
      ...
      <span
        style={{
          fontSize: 12,
          color: optimisticDone ? T.dim : T.muted,
...
function CreatorRow({ c, i }: { c: CreatorData; i: number }) {
  ...
  function updateShipping(val: string) {
    setCurrentShipping(val);
        ...
        <td style={cell}>
          <span
            style={{
              ...
              padding: "2px 7px",
              borderRadius: 99,
              background: isRefuse ? "#f9fafb" : "#eef2ff",
              color: isRefuse ? T.muted : T.accent,
            }}
          >
            {typeLabel}
          </span>
        ...
        <td style={{ ...cell, whiteSpace: "nowrap" }}>
          <div style={{ display: "flex", gap: 5, alignItems: "center" }}>
            {isRefuse ? (
              <button
                type="button"
                onClick={() => updateShipping("en_attente")}
                style={{
                  fontSize: 11,
                  padding: "3px 8px",
                  borderRadius: 6,
...
export default function UGCPage() {
  ...
  const submitting = createFetcher.state === "submitting";
      ...
      <div style={{ maxWidth: 1160, margin: "0 auto" }}>
                ...
                <thead>
                  <tr style={{ background: "#f1f5f9" }}>
                    {["Nom", "Pays", "Produit brut", "→ Clé", "Composants", "Port calculé", "Statut colis"].map((h) => (
                      <th key={h} style={{ padding: "7px 10px", textAlign: "left", fontWeight: 600, color: T.muted, whiteSpace: "nowrap", borderBottom: `1px solid ${T.border}` }}>
                        {h}
                      </th>
                    ))}
                  </tr>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 24 }}>
          <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: "14px 18px", boxShadow: T.shadow }}>
            <div style={{ fontSize: 10, fontWeight: 600, textTransform: "uppercase", color: T.dim, letterSpacing: "0.08em", marginBottom: 10 }}>Pipeline colis</div>
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
              {(["en_attente", "preparation", "envoye", "livre"] as const).map((s, idx, arr) => (
                <div key={s} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <div style={{ textAlign: "center" }}>
                    <div style={{ ...shippingStyle(s), borderRadius: 99, padding: "3px 12px", fontSize: 12, fontWeight: 700, display: "inline-block" }}>{totaux[`ship_${s}` as keyof typeof totaux]}</div>
          ...
          <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: "14px 18px", boxShadow: T.shadow }}>
            <div style={{ fontSize: 10, fontWeight: 600, textTransform: "uppercase", color: T.dim, letterSpacing: "0.08em", marginBottom: 10 }}>Pipeline contenu</div>
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
              {(["a_faire", "recu", "poste"] as const).map((s, idx, arr) => (
                <div key={s} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <div style={{ textAlign: "center" }}>
                    <div style={{ ...contentStyle(s), borderRadius: 99, padding: "3px 12px", fontSize: 12, fontWeight: 700, display: "inline-block" }}>{totaux[`cont_${s}` as keyof typeof totaux]}</div>
        ...
        <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 16, padding: 24, marginBottom: 28, boxShadow: T.shadow }}>
          <div style={{ fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", color: T.muted, marginBottom: 16 }}>Ajouter un créateur</div>

          <createFetcher.Form method="post" action=".">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 12 }}>
              ...
              <label style={lbl}>
                <span style={lbT}>Pays</span>
                <select name="pays" required style={inp}>
                  {Object.entries(PAYS_LABELS).map(([v, l]) => (
                    <option key={v} value={v}>
                      {l}
                    </option>
                  ))}
              ...
              <label style={lbl}>
                <span style={lbT}>Frais port auto si vide</span>
              ...
              <label style={lbl}>
                <span style={lbT}>Code promo</span>
          ...
          <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 16, overflow: "hidden", boxShadow: T.shadow }}>
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                ...
                <tbody>
                  {creators.map((c, i) => (
                    <CreatorRow key={c.id} c={c} i={i} />
                  ))}
                </tbody>

                <tfoot>
                  <tr style={{ background: "#f1f5f9", borderTop: `2px solid ${T.border}` }}>
                    <td colSpan={4} style={{ padding: "10px 10px", fontWeight: 700, color: T.text }}>
                      Total
                    </td>
