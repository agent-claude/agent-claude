...
import { fetchShippingOrders } from "../utils/shopify-orders.server";
import {
  listOrderEmailLogs,
  upsertOrderEmailLog,
  saveTracking,
...
// Laya premium palette: cream paper + deep aubergine.
const T = {
  bg: "#FBF8F4",
  card: "#FFFFFF",
  cardSoft: "#F8F2EA",
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  let realSnapshots: OrderSnapshot[] = [];
  let fetchFailed = false;

  if (!demoForced) {
    try {
      const result = await fetchShippingOrders(admin, 50);
      realSnapshots = result.snapshots;
    } catch (err) {
      console.warn("[shipping] Shopify fetch failed, switching to demo mode:", err);
      fetchFailed = true;
    }
  }

  const isDemo = demoForced || fetchFailed || realSnapshots.length === 0;
  const snapshots = isDemo ? getDemoSnapshots() : realSnapshots;
        ...
        <header style={{ marginBottom: 24 }}>
          ...
          <p style={{ margin: "6px 0 0", fontSize: 13, color: T.muted }}>
            {isDemo
              ...
              : "Préparation des emails de confirmation et d'expédition. Aucun envoi réel en Phase 1."}
          </p>
        </header>
...
function PreviewModal({
  ...
  const src = `/app/shipping/preview?logId=${encodeURIComponent(logId)}&type=${type}`;
